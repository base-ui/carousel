...
function Carousel(el) {
  ...
  function resizeSlideList() {
    ...
    var slidesWidth, slidesHeight;

    if (slides.length === 0) {
      slidesWidth = 0;
      slidesHeight = 0;
    } else {
      slidesWidth = slides[0].offsetWidth * slides.length;
      slidesHeight = slides[0].offsetHeight * slides.length;
    }

    if (thisCarousel.slideList.direction === 'horizontal') {
      j(thisCarousel.slideList.el).width(slidesWidth);
    }

    if (thisCarousel.slideList.direction === 'vertical') {
      j(thisCarousel.slideList.el).height(slidesHeight);
    }
  }
}
